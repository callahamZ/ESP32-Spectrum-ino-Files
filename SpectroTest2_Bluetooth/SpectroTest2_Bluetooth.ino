...
// Check if Bluetooth is properly enabled in your ESP32 configuration
...
void setup() {
  ...
  SerialBT.begin("MySpectrum ESP32 AS7341"); // Set your desired Bluetooth device name
  Serial.println("Bluetooth Serial started");

  Serial.println("WiFi functionality removed. Data will be sent via USB Serial and Bluetooth Serial.");
...
void loop() {
  ...
  if (buttonState == LOW && !buttonPressed)
  {
    ...
    // SerialBT.println("Button Pressed"); // Optional: Send button press notification via Bluetooth
    ...
    if (!as7341.readAllChannels(readings))
    {
      ...
      SerialBT.println("Error reading all channels!"); // Also send error via Bluetooth
      return;
    }
    ...
    if (SerialBT.connected()) { // Check if a Bluetooth device is connected
      ...
      // Optional: You can print a message to USB Serial if no BT device is connected
      // Serial.println("Bluetooth not connected, data not sent via BT.");
    }
  }
